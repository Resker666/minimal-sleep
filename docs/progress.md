...
2026-09-22 构建流水线：新增 `.github/workflows/android-apk.yml`，在 `main` 与 `codex/**` 推送时运行 Python 音频工具测试、Android Lint、JVM 单元测试并构建调试 APK，成功后上传 APK 和 SHA-256 文件为 14 天的 Actions 产物。云端实际运行结果以 [验证记录](validation.md) 为准；工作流不发布 GitHub Release。
