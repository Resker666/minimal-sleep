...
`source/` 中的原始 MP3 保持本地、被 `.gitignore` 排除，原件不修改也不直接入库。用户于 2026-09-22 确认其中 `rain-01.ogg`、`rain-04.ogg` 对应的音轨由本人录制，并以 Resker666 署名按 CC BY 4.0 授权剪辑和公开分发；其余五段候选的权利状态未确认，仍只供本地试听。文件名中的视频编号和“真实”字样本身不能证明权利。
...
私有调试 APK 为 `deliverables/minimal-sleep-v0.3.1-local-rain-debug.apk`。当时权利声明未齐全，因此只供本机安装和听感验证，没有公开发布。`rain-01.ogg` 已在真机播放跨过一次 298 秒边界，媒体会话维持 `PLAYING` 并从约 279 秒回到约 17 秒；这只能证明功能性循环。`rain-04.ogg` 的完整循环、两段的接缝听感、主观响度和整夜播放仍需试听。随后用户作出了上述作者与 CC BY 4.0 授权声明，当前版本将两段剪辑移入主资源并置于声音列表最前面。

## 内置前的门槛

为其他拟内置音频保存作者、来源、明确允许修改并随 APK 分发的许可或授权、署名要求和文件哈希。当前仓库的 Apache-2.0 源码许可证不覆盖按 CC BY 4.0 提供的两段雨声剪辑。其他音频没有许可证明时，继续采用用户在手机上本地导入的方式。
