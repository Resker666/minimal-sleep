...
- 本地 iPhone 18 Pro / iOS 27.0 模拟器上 90 个 XCTest 通过，未签名 Release 模拟器构建成功；App 包中的版本、麦克风用途说明、后台 audio 和两段 M4A 加三段小型 WAV 已检查。自动测试覆盖状态、切片、文件事务和恢复，但不能证明真机麦克风或锁屏录音。
